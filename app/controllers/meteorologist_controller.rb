...
class MeteorologistController < ApplicationController
  ...
  def street_to_weather
    ...
    parsed_data = JSON.parse(open(url_loc).read)

    @lat = parsed_data["results"][0]["geometry"]["location"]["lat"]

    @lng = parsed_data["results"][0]["geometry"]["location"]["lng"]

    url_fcast = "https://api.forecast.io/forecast/4c161d79ae009ea2445d7191e4a6f2cc/" + @lat.to_s + "," + @lng.to_s

    parsed_data_forecast = JSON.parse(open(url_fcast).read)
